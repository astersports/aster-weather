...
import { skyConditionFor, type SkyCondition } from "./skyTints.js";
import { ensureSkyStyles } from "./skyStyles.js";

export interface WeatherIconProps {
  /** WMO-derived icon key — `getWeatherInfo(code).icon`. */
  condition: WeatherIconKey;
  /** Day vs night art (clear / partly-cloudy / snow get night variants). Default true. */
  isDay?: boolean;
  /** Ambient motion. Default TRUE. `prefers-reduced-motion` always wins. */
  animate?: boolean;
  /** When set, exposes the icon to the a11y tree (role=img + <title>); else aria-hidden. */
  title?: string;
  className?: string;
...
export function WeatherIcon({ condition, isDay = true, animate = true, title, className, style }: WeatherIconProps) {
  const uid = React.useId().replace(/:/g, "");
  React.useEffect(() => { ensureSkyStyles(); }, []);
  const cond: SkyCondition = skyConditionFor(condition, isDay);
  ...
  return (
    <svg
      ...
      {...a11y}
      dangerouslySetInnerHTML={{ __html: (title ? `<title>${escapeXml(title)}</title>` : "") + art }}
    />
  );
}
