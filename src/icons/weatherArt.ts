...
const CLOUD =
  "M20 46 C12.6 46 7 40.6 7 34 C7 27.9 11.7 22.8 17.8 22.1 C19.6 15.3 25.8 10.4 33 10.4 " +
  "C40.4 10.4 46.6 15.5 48.3 22.3 C54 23.1 58 27.9 58 33.9 C58 40.6 52.6 46 45.4 46 Z";

function cloud(id: string, dx: number, dy: number, sc: number, cls: string, dark: boolean, al?: number): string {
  const s = dark ? { a: "#B4C2D6", b: "#8D9DB6", c: "#65758F" } : { a: "#FDFEFF", b: "#E1E9F5", c: "#B3C2D8" };
  return (
    `<defs><linearGradient id="cg${id}" x1=".15" y1="0" x2=".55" y2="1">` +
...
function sun(id: string, cx: number, cy: number, r: number, a: boolean, rays = true): string {
  let ry = "";
  if (rays) {
    for (let i = 0; i < 8; i++) {
  ...
  return (
    `<defs><radialGradient id="sd${id}" cx="34%" cy="28%">` +
...
function moon(id: string, cx: number, cy: number, r: number, a: boolean): string {
  const d =
    `M ${cx + r * 0.36} ${cy - r} a ${r} ${r} 0 1 0 ${r * 0.02} ${2 * r}` +
    ` a ${r * 1.08} ${r * 1.08} 0 1 1 ${-r * 0.02} ${-2 * r} Z`;
  let st = "";
  ([[cx + r * 1.35, cy - r * 1, 1.5], [cx + r * 1.85, cy - r * 0.28, 1.05], [cx + r * 1.2, cy + r * 0.42, 0.9]] as const).forEach((p, i) => {
    st += `<circle cx="${p[0]}" cy="${p[1]}" r="${p[2]}" fill="#FFF0BC" class="${a ? "aw-twinkle" : ""}" style="animation-delay:${i * 0.8}s"/>`;
  });
  return (
    `<defs><linearGradient id="mg${id}" x1=".2" y1="0" x2=".8" y2="1">` +
...
function drops(id: string, xs: number[], y: number, h: number, w: number, a: boolean, sp: string): string {
  let o =
    `<defs><linearGradient id="dg${id}" x1="0" y1="0" x2=".3" y2="1">` +
    `<stop offset="0%" stop-color="#DCEEFF"/><stop offset="45%" stop-color="#8FC4F0"/>` +
    `<stop offset="100%" stop-color="#5591CE"/></linearGradient></defs>`;
  xs.forEach((x, i) => {
    const d =
      `M ${x} ${y} C ${x + w} ${y + h * 0.55} ${x + w} ${y + h} ${x} ${y + h}` +
...
function flake(id: string, x: number, y: number, r: number, a: boolean, dl: number, w: number): string {
  let o = "";
  for (let i = 0; i < 3; i++) {
...
function bolt(id: string, a: boolean): string {
...
function fogBars(a: boolean): string {
  let b = "";
  [0, 1, 2].forEach((i) => {
...
const dr = (a: boolean) => (a ? "aw-drift" : "");

export const WEATHER_ART: Record<SkyCondition, WeatherArtFn> = {
