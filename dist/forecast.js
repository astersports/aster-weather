 ...
 */
export async function fetchForecast(coords, opts = {}) {
 ...
 */
export function getWeatherForTime(hours, isoTime, windowMs = HOURLY_MATCH_WINDOW_MS) {
    if (!hours || !hours.length || isoTime == null)
        return null;
...
function stripLabel(timestamp, utcOffsetSeconds) {
...
export async function getWeatherForEvent(coords, eventStartISO, opts = {}) {
    ...
    const stripForecasts = hours
        .filter((f) => f.timestamp >= eventTime - 2 * 60 * 60 * 1000 &&
